...
//这次还要考虑每个t中字符出现的次数。。
//用一个needNums[]记录每个字符需要的次数
public class findSonMinWindow_DaXiaoXie_Cishu {
    public String minWindow(String s, String t) {
        ...
        for(char c:chars_t){
            ...
            needNums[charIndex]++; //记录每个字符需要出现的次数,用于和window(记录已经出现的次数)作比较
        }
        ...
        while(right<chars_s.length){
            ...
            boolean flag=true;
            for(int j:needIndex){
                if(window[j]<needNums[j]){ //如果已有的字符数量没超过需要的数量
                    flag=false;
                    break;
                }
            }
            if(flag){  //如果全了，判断长度小于当前就替代result
                ...
                while(!need[chars_s[left]-'A']||window[chars_s[left]-'A']>needNums[chars_s[left]-'A']){
                    if(!need[chars_s[left]-'A']){ //如果是非必要的字符，left++就行
                        left++;
                    }
                    if(window[chars_s[left]-'A']>needNums[chars_s[left]-'A']){ //如果是必要的冗余字符，left++且统计减少一
